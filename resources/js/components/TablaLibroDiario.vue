<template>
    <div>
        <h5 class="text-center mt-2">
            Libro#: {{ LibroDiario.id }}
            <v-btn
                :href="libroImprimir"
                color="red"
                class=" white--text float-right text-decoration-none"
                small
            >
            Descargar
                <v-icon right small>
        ...
        <table class="table table-hover mt-3">
            ...
            <tbody>
                ...
                >
                    <th scope="row">
                        {{ transaccion.cuentaId }} {{ transaccion.subcuenta }}
                    </th>
...
export default {
    ...
    methods: {
        ...
        cambiarEstado() {
            //se ejecuta cuando actualizo un registro
        ...
        eliminarPartida(registro) {
            axios.delete(`/registrold/${registro.transaccion}`).then(res => {
                this.$emit("actualizarPartidas", this.LibroDiario);
            });
        }
    },
    computed: {
